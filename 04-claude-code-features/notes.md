...
| 模式 | 行为 | 适合场景 |
|------|------|---------|
| `default` | 每次危险操作都问你 | 日常使用，默认推荐 |
| `acceptEdits` | 自动接受文件编辑，其他仍问 | 信任的编程任务 |
| `auto` | 大部分操作自动执行 | 你很信任这个任务时 |
| `plan` | 只规划不执行，全部操作都要确认 | 陌生代码库，谨慎模式 |
...
- **最小权限原则：** 只给任务需要的权限
- **高风险操作要确认：** rm、git push、数据库操作
- **不要在生产环境用 auto 模式**
- **MCP 连接高风险系统时要格外谨慎**（银行、数据库）
